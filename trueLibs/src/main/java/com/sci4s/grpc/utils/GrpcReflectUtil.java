...
//							logger.info("&&&&&&&&&&&&&&&&&&&");						
	...
	public static SciRIO.RetMsg callServiceForGrpcParams(Object beanObj, String queryId, String methName, GrpcParams grpcPrms) throws Exception {
		SciRIO.RetMsg grpcResp = null;
		Method method    = null;
		try {
			if ("N".equals(queryId)) {	
				Class prmTypes[] = { GrpcParams.class };
				method = beanObj.getClass().getDeclaredMethod(methName, prmTypes);
			...
			if (grpcResp != null) { try { grpcResp = null; } catch (Exception ex) {} }
			if (method   != null) { try { method   = null; } catch (Exception ex) {} }
		}
	}
	
	public static SciRIO.RetMsg callServiceForMap(Object beanObj, String queryId, String methName, Map<String, String> paramsMap) throws Exception {
		SciRIO.RetMsg grpcResp = null;
		Method method    = null;
		try {
			if ("N".equals(queryId)) {	
				...
				method = beanObj.getClass().getDeclaredMethod(methName, prmTypes);
				grpcResp = (SciRIO.RetMsg) method.invoke(beanObj, new Object[] { paramsMap });
			} else {			
				Class prmTypes[] = { String.class, Map.class };
				method = beanObj.getClass().getDeclaredMethod(methName, prmTypes);
				grpcResp = (SciRIO.RetMsg) method.invoke(beanObj, new Object[] { queryId, paramsMap });
			}
			return grpcResp;
		} catch (Exception e) {
			throw e;
